...
from config.ascii_banner import print_banner
...
class Netbreach:
    ...
    def print_results(self, results, output, number):
        print(f'Selecting the first {len(results)} results....Done.\n')
        ...
        for line in results:
            parts = line.split(":")
            [print(parts) if GENERAL_SETTINGS.SHOW_RAW_JSON_ENTRIES else None]
            if len(parts) == 2:
